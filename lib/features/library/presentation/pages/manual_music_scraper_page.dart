...
class _ManualMusicScraperPageState extends ConsumerState<ManualMusicScraperPage> {
  ...
  void _sortByDurationMatch(List<MusicScraperItem> items, Map<ScraperType, int> sourcePriorities) {
  ...
  double _getMatchPercent(MusicScraperItem item) {
    if (_musicDurationMs <= 0 || item.durationMs == null || item.durationMs == 0) return 0;
  ...
  String _formatDurationDiff(MusicScraperItem item) {
    if (_musicDurationMs <= 0 || item.durationMs == null || item.durationMs == 0) return '';
    ...
    try {
      ...
      if (item.source.supportsLyrics) {
        ...
        if (source != null) {
          ...
          if (scraper != null) lyrics = await scraper.getLyrics(item.externalId);
        }
      }

      CoverScraperResult? cover;
      if (item.coverUrl != null) {
        cover = CoverScraperResult(source: item.source, coverUrl: item.coverUrl!);
      }
  ...
  Future<void> _confirmAndScrape() async {
    if (_selectedDetail == null && _selectedCover == null && _selectedLyrics == null) return;

    final filePath = widget.music.filePath;
    if (filePath == null) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('无法获取文件路径'), backgroundColor: Colors.red),
        );
      }
      return;
    }
    ...
    try {
      ...
      if (_downloadCover && _selectedCover != null) {
        ...
        if (coverData != null) await _saveCoverFile(musicDir, baseName, coverData);
      }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(children: [
          _buildHeader(isDark),
          _buildCompactFileInfo(isDark),
          _buildSearchBar(isDark),
          Expanded(child: _buildSearchResults(isDark)),
          if (_selectedItem != null) _buildSelectionPanel(isDark),
        ]),
      ),
    );
  }
  ...
  Widget _buildHeader(bool isDark) => Padding(
        ...
        child: Row(children: [
          ...
          Expanded(child: Text('手动刮削', style: TextStyle(
            color: context.yyTextPrimary, fontSize: 20, fontWeight: FontWeight.w800, letterSpacing: -0.5))),
          if (_totalResultCount > 0)
            Text('$_totalResultCount 个结果', style: TextStyle(color: context.yyTextTertiary, fontSize: 13)),
        ]),
      );

  Widget _buildCompactFileInfo(bool isDark) => Container(
        ...
        child: Row(children: [
          Container(
            ...
            child: Icon(CupertinoIcons.music_note, color: YYColors.accentPrimary, size: 18),
          ),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              if (widget.music.artist.isNotEmpty)
                Flexible(child: Text(widget.music.artist, maxLines: 1, overflow: TextOverflow.ellipsis,
              ...
              if (_musicDurationMs > 0) ...[
                ...
                Container(
                  ...
                  child: Text(_formatDuration(_musicDurationMs),
                    style: TextStyle(fontSize: 10, color: YYColors.accentPrimary, fontWeight: FontWeight.w500)),
                ),
              ],
            ]),
          ])),
        ...
        child: Row(children: [
          Expanded(
            ...
            child: CupertinoTextField(
              ...
              prefix: Padding(
                ...
                child: Icon(CupertinoIcons.music_note, size: 16, color: context.yyTextTertiary),
              ),
              ...
              placeholderStyle: TextStyle(color: context.yyTextTertiary, fontSize: 14),
          ...
          Expanded(
            ...
            child: CupertinoTextField(
              ...
              prefix: Padding(
                padding: const EdgeInsets.only(left: 10),
                child: Icon(CupertinoIcons.person, size: 16, color: context.yyTextTertiary),
              ),
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10),
              decoration: BoxDecoration(
                color: isDark ? Colors.white.withValues(alpha: 0.06) : Colors.grey[100],
  ...
  Widget _buildSearchResults(bool isDark) {
    if (_isSearching) return Center(child: CupertinoActivityIndicator(radius: 14, color: context.yyTextSecondary));

    if (_errorMessage != null) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(CupertinoIcons.exclamationmark_triangle, size: 48, color: Colors.red[400]),
        const SizedBox(height: 16),
        Text(_errorMessage!, textAlign: TextAlign.center, style: TextStyle(color: context.yyTextSecondary)),
        const SizedBox(height: 16),
        CupertinoButton(color: YYColors.accentPrimary, onPressed: _search,
    ...
    if (_searchResults.isEmpty) {
      return Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
        Icon(CupertinoIcons.search, size: 48, color: context.yyTextTertiary),
        const SizedBox(height: 12),
        Text('未找到结果', style: TextStyle(color: context.yyTextSecondary, fontSize: 16, fontWeight: FontWeight.w600)),
        const SizedBox(height: 4),
        Text('尝试调整搜索关键词', style: TextStyle(color: context.yyTextTertiary, fontSize: 13)),
      ]));
    }
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return _buildResultCard(item, isSelected, isDark);
      },
    );
  }

  Widget _buildResultCard(MusicScraperItem item, bool isSelected, bool isDark) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: isSelected
              ? YYColors.accentPrimary.withValues(alpha: isDark ? 0.15 : 0.08)
        ...
        child: Row(children: [
          ...
          Container(
            ...
            child: Stack(children: [
              if (item.coverUrl != null)
                ClipRRect(
                  borderRadius: BorderRadius.circular(8),
                  child: Image.network(item.coverUrl!, fit: BoxFit.cover, width: 48, height: 48,
                    errorBuilder: (_, __, ___) => Center(
                      child: Icon(CupertinoIcons.music_note, color: item.source.themeColor, size: 20))),
                )
              else
                Center(child: Icon(CupertinoIcons.music_note, color: item.source.themeColor, size: 20)),
              Positioned(right: 0, bottom: 0, child: Container(
                padding: const EdgeInsets.all(2),
                decoration: BoxDecoration(
                  color: item.source.themeColor,
                  borderRadius: const BorderRadius.only(topLeft: Radius.circular(6), bottomRight: Radius.circular(8)),
                ),
                child: Icon(item.source.icon, size: 9, color: Colors.white),
              )),
            ]),
          ),
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text([if (item.artist != null) item.artist!, if (item.album != null) item.album!].join(' · '),
            ...
            Row(children: [
              Container(
                ...
                child: Text(item.source.displayName,
                  style: TextStyle(fontSize: 9, color: item.source.themeColor, fontWeight: FontWeight.w500)),
              ),
              const SizedBox(width: 6),
              if (item.durationText.isNotEmpty)
                Text(item.durationText, style: TextStyle(fontSize: 10, color: context.yyTextTertiary)),
              if (item.source.supportsLyrics) ...[
                const SizedBox(width: 6),
                Icon(CupertinoIcons.text_quote, size: 11, color: Colors.cyan[isDark ? 300 : 700]),
              ],
            ]),
          ])),
          // 匹配度
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  color: hasHighMatch
                      ...
                      : (matchPercent >= 50 ? Colors.orange.withValues(alpha: 0.15) : Colors.grey.withValues(alpha: 0.15)),
                  borderRadius: BorderRadius.circular(6),
                ),
                child: Column(children: [
                  Text('${matchPercent.toInt()}%', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold,
                    color: hasHighMatch ? Colors.green : (matchPercent >= 50 ? Colors.orange : Colors.grey))),
                  if (durationDiff.isNotEmpty)
                    Text(durationDiff, style: TextStyle(fontSize: 9,
                      color: hasHighMatch ? Colors.green : (matchPercent >= 50 ? Colors.orange : Colors.grey))),
                ]),
              )
            else
              Icon(CupertinoIcons.chevron_right, size: 16, color: context.yyTextTertiary),
            if (isSelected)
              Padding(padding: const EdgeInsets.only(top: 4),
                child: Icon(CupertinoIcons.checkmark_circle_fill, color: YYColors.accentPrimary, size: 20)),
          ]),
        ]),
      ),
    );
  ...
  Widget _buildSelectionPanel(bool isDark) {
    ...
    return Container(
      decoration: BoxDecoration(
        color: isDark ? context.yyBgElevated : Colors.white,
      ...
      child: SafeArea(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          if (_isLoadingDetail)
            LinearProgressIndicator(color: YYColors.accentPrimary, minHeight: 2)
          else if (hasContent)
            Padding(
              ...
              child: Column(children: [
                ...
                Row(children: [
                  ...
                  const SizedBox(width: 12),
                  Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    Text(_selectedDetail?.title ?? _selectedItem?.title ?? '', maxLines: 1,
                    ...
                    Row(children: [
                      Container(
                        padding: const EdgeInsets.symmetric(horizontal: 5, vertical: 1),
                        decoration: BoxDecoration(
                          color: _selectedItem?.source.themeColor.withValues(alpha: 0.12),
                          borderRadius: BorderRadius.circular(3),
                        ),
                        child: Text(_selectedItem?.source.displayName ?? '',
                          style: TextStyle(fontSize: 9, color: _selectedItem?.source.themeColor)),
                      ),
                      if (_selectedCover != null) ...[
                        const SizedBox(width: 4),
                        _buildFeatureChip(CupertinoIcons.photo, '封面', isDark),
                      ],
                      if (_selectedLyrics?.hasLyrics ?? false) ...[
                        const SizedBox(width: 4),
                        _buildFeatureChip(CupertinoIcons.text_quote, '歌词', isDark),
                      ],
                    ]),
                  ])),
                ]),
                ...
                // 选项
                Row(children: [
                  Expanded(child: _buildCompactOption('封面${_hasCover ? "(覆盖)" : ""}',
                    _downloadCover && _selectedCover != null,
                    _selectedCover != null ? (v) => setState(() => _downloadCover = v) : null, isDark)),
                  const SizedBox(width: 8),
                  Expanded(child: _buildCompactOption('歌词${_hasLyrics ? "(覆盖)" : ""}',
                    _downloadLyrics && (_selectedLyrics?.hasLyrics ?? false),
                    (_selectedLyrics?.hasLyrics ?? false) ? (v) => setState(() => _downloadLyrics = v) : null, isDark)),
                ]),
                const SizedBox(height: 12),
                // 按钮
                Row(children: [
                  Expanded(child: GestureDetector(
                    onTap: _isScraping ? null : () => setState(_clearSelection),
                    child: Container(
                      ...
                      child: Center(child: Text('取消', style: TextStyle(
                        color: context.yyTextSecondary, fontSize: 14, fontWeight: FontWeight.w500))),
                    ),
                  )),
                  const SizedBox(width: 12),
                  Expanded(flex: 2, child: GestureDetector(
                    onTap: _isScraping ? null : _confirmAndScrape,
                    child: Container(
                      padding: const EdgeInsets.symmetric(vertical: 11),
                      decoration: BoxDecoration(
                        color: YYColors.accentPrimary,
                      ...
                      child: Center(child: _isScraping
                          ? const SizedBox(width: 18, height: 18,
                              child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                          : const Row(mainAxisSize: MainAxisSize.min, children: [
                              Icon(CupertinoIcons.checkmark, size: 16, color: Colors.white),
                              SizedBox(width: 6),
                              Text('确认刮削', style: TextStyle(color: Colors.white, fontSize: 14, fontWeight: FontWeight.w600)),
                            ])),
                    ),
                  )),
                ]),
            ...
            Padding(
              ...
              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: context.yyTextTertiary)),
                const SizedBox(width: 12),
                Text('正在获取详情...', style: TextStyle(color: context.yyTextSecondary, fontSize: 14)),
              ]),
            ),
        ]),
      ),
  ...
  Widget _buildFeatureChip(IconData icon, String label, bool isDark) => Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          ...
          Text(label, style: TextStyle(fontSize: 9, color: isDark ? Colors.green[300] : Colors.green[700])),
        ]),
      );

  Widget _buildCompactOption(String label, bool value, void Function(bool)? onChanged, bool isDark) {
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(8),
          border: Border.all(color: value ? YYColors.accentPrimary : Colors.transparent),
        ),
        child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(
          ...
          Flexible(child: Text(label, maxLines: 1, overflow: TextOverflow.ellipsis,
            style: TextStyle(fontSize: 11, color: isEnabled ? context.yyTextPrimary : Colors.grey[400]))),
        ]),
      ),
    );
  }
