          ...
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              title: Text(widget.title, style: TextStyle(
                  color: context.yyTextPrimary, fontWeight: FontWeight.bold, fontSize: 18)),
              background: Container(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    begin: Alignment.topCenter, end: Alignment.bottomCenter,
                    colors: [YYColors.accentPrimary.withValues(alpha: 0.15), context.yyBgBase],
                  ),
                ),
              ),
            ),
            actions: [
              if (_songs.isNotEmpty)
                PopupMenuButton<_SortMode>(
                  ...
                  itemBuilder: (_) => [
                    _sortMenuItem(_SortMode.original, '默认顺序'),
                    _sortMenuItem(_SortMode.title, '按标题'),
                    _sortMenuItem(_SortMode.artist, '按艺术家'),
                    _sortMenuItem(_SortMode.duration, '按时长'),
                  ],
                ),
              if (_songs.isNotEmpty)
                IconButton(
    ...
    return PopupMenuItem(
      ...
      child: Row(
        children: [
          ...
          Text(label, style: TextStyle(color: context.yyTextPrimary)),
        ],
      ),
    );
  }
          ...
          children: [
            SizedBox(
              ...
              child: isPlaying
                  ...
                  : Text('$index', style: TextStyle(color: context.yyTextTertiary, fontSize: 14), textAlign: TextAlign.center),
            ),
